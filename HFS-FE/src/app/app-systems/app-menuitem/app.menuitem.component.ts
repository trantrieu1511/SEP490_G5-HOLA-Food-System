    ...
    template: `
        <ng-container>
            ...
            <a [attr.href]="item.url" (click)="itemClick($event)" *ngIf="(!item.routerLink || item.items) && item.visible !== false" (keydown.enter)="itemClick($event)"
               ...
               [pTooltip]="item.label" [tooltipDisabled]="active || !(root && layoutService.isSlim() && !layoutService.isMobile())">
...
export class AppMenuitemComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        if (!(this.layoutService.isHorizontal() || this.layoutService.isSlim()) && this.item.routerLink) {
            this.updateActiveStateFromRoute();
        }
    ...
    itemClick(event: Event) {
        ...
        if (this.item.items) {
            ...
            if (!this.layoutService.isStatic()) {
    ...
    onMouseEnter() {
        ...
        if (this.root  && (this.layoutService.isHorizontal() || this.layoutService.isSlim()) && this.layoutService.isDesktop()) {
